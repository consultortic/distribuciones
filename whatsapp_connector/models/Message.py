...
class AcruxChatMessages(models.Model):
    ...
    def get_fields_to_read(self):
        return ['id', 'text', 'ttype', 'date_message', 'from_me', 'res_model',
                'res_id', 'error_msg', 'show_product_text']
    ...
    def message_parse(self):
        ...
        message.update({
            'to': self.connector_id.clean_id(self.contact_id.number),
            'id': str(self.id),
        })
        return message
    ...
    @api.model
    def message_check_time(self, contact_id, raise_on_error=True):
    ...
    def message_check_allow_send(self):
        ...
        if connector_id.connector_type == 'gupshup':
            self.message_check_time(self.contact_id)
    ...
    def add_attachment(self, data):
        ...
        if url.startswith('http'):
            try:
                attach_id = create_attachment_from_url(self.env, url, self, data.get('filename'))
