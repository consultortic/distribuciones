...
class AcruxChatConversation(models.Model):
    ...
    def event_create(self, event, user_id=False):
    ...
    def _compute_number_format(self):
        for rec in self:
            rec.number_format = rec.connector_id.format_id(rec)
    ...
    def new_message(self, data):
        ...
        if send_bus:
            ...
            self.env['bus.bus'].sendone(conversation.get_bus_channel(),
                                        {'new_messages': data_to_send})
        return message_id
    ...
    def get_channel_to_many(self):
        self.ensure_one()
        return (self.env.cr.dbname, self._name, self.connector_id.company_id.id)

    def get_channel_to_one(self, user_id=None):
        self.ensure_one()
        if not user_id:
            user_id = self.agent_id
        return (self.env.cr.dbname, self._name, self.connector_id.company_id.id, user_id.id)
    ...
    def update_conversation_bus(self):
        ...
        self.env['bus.bus'].sendone(self.get_channel_to_many(),
                                    {'update_conversation': data_to_send})
    ...
    def new_message_event(self, connector_id, msgid, data):
        ...
        if message_id:
            ...
            if data['type'] == 'failed':
                self.env['bus.bus'].sendone(conv_id.get_bus_channel(),
                                            {'error_messages': data_to_send})
            else:
                self.env['bus.bus'].sendone(conv_id.get_channel_to_many(),
                                            {'update_conversation': data_to_send})
        return message_id
    ...
    def send_message_bus_release(self, msg_data, back_status, check_access=True):
        ...
        if back_status == 'new':
            conv_id.set_to_new()
            to_bus = True
            data_to_send = conv_id.build_dict(limit=22)
    ...
    def get_fields_to_read(self):
        return ['id', 'name', 'agent_id', 'status', 'team_id', 'image_url',
                ...
                'last_activity', 'desk_notify', 'connector_type', 'show_icon']
    ...
    def conversation_verify(self):
        ...
        for conn_id in Connector.search([]):
            ...
            if len(all_ids):
                ...
                to_done.set_to_done()
                to_news.set_to_new()
                data_to_send = {'delete_taken_conversation': conv_delete_ids,
                                'delete_conversation': conv_delete_ids,
                                'new_messages': to_news.build_dict(22)}
                notifications.append((all_ids[0].get_channel_to_many(), data_to_send))
        if notifications:
    ...
    def block_conversation(self):
        ...
        if self.status in ['new', 'done']:
            ...
            if back_status == 'new':
                self.env['bus.bus'].sendone(channel, {'delete_conversation': [data_to_send]})
        else:
    ...
    def contact_update(self, connector_id, data):
        ...
        if number and (name or image_url):
