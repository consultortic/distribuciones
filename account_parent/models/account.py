...
class AccountAccountTemplate(models.Model):
    ...
    def _search(
        ...
        if args:
            for arg in args:
                if (
                    ...
                    new_args.append(arg)
        # one Customer informed an issue that the same args is
        # updated to company causing error
        ...
        return super(AccountAccountTemplate, self)._search(
            new_args,
            offset=offset,
            limit=limit,
            order=order,
...
class AccountAccount(models.Model):
    ...
    def _compute_account_root(self):
        ...
        # This field should have been a char, but the aim is to use it in a side
        # panel view with hierarchy, and it's only supported by many2one fields so far.
        ...
        for record in self:
            if record.user_type_id.type != "view":
                record.root_id = (
                    record.code
                    and (ord(record.code[0]) * 1000 + ord(record.code[1]))
                    or False
                )
            else:
    ...
    def _compute_values(self):
        for account in self:
            ...
            query1 = (
                "SELECT account_move_line.debit,account_move_line.credit FROM "
                + tables
                + "WHERE"
                + where_clause
            )
            self.env.cr.execute(SQL(query1), tuple(where_params))
            ...
            if context.get("show_initial_balance"):
                ...
                self.env.cr.execute(SQL(query2), tuple(where_params))
                for deb, cre in self.env.cr.fetchall():
                    initial_cre += cre
                    initial_deb += deb
    ...
    balance = fields.Float(compute="_compute_values", digits=(16, 4), string="Balance")
    credit = fields.Float(compute="_compute_values", digits=(16, 4), string="Credit")
    debit = fields.Float(compute="_compute_values", digits=(16, 4), string="Debit")
    parent_id = fields.Many2one(
    ...
    initial_balance = fields.Float(
        compute="_compute_values", digits=(16, 4), string="Initial Balance"
    )
        ...
        if args:
            for arg in args:
                if (
                    ...
                    new_args.append(arg)
        # one Customer informed an issue that the same args is updated to company
        # causing error
        # So to avoid that args was copied to new variable and it solved the issue.
        if not context.get("show_parent_account", False):
            new_args = expression.AND([[("user_type_id.type", "!=", "view")], new_args])
...
class AccountJournal(models.Model):
    ...
    @api.model
    def _prepare_liquidity_account(self, name, company, currency_id, type_1):
        res = super(AccountJournal, self)._prepare_liquidity_account(
            name, company, currency_id, type_1
        )
        if type_1 == "bank":
