...
class AccountChartTemplate(models.Model):
    ...
    def generate_account(
        ...
        ).generate_account(tax_template_ref, acc_template_ref, code_digits, company)
    ...
    def update_generated_account(
        self,
        tax_template_ref=None,
        code_digits=1,
        company=False,
        ...
        account_tmpl_obj = self.env["account.account.template"].with_context(
            {"show_parent_account": True}
        )
        ...
        for account_template in acc_templates:
            ...
            if not new_account:
                vals = self._get_account_vals(
                    company, account_template, new_code, tax_template_ref
                )
        ...
        for account_template in acc_templates:
            ...
            account_xml_id = "%s.%s_%s" % (
                template_xml_obj.module,
                company.id,
                template_xml_obj.name,
            )
