odoo.define("account_parent.CoAWidget", function (require) {
    ...
    var CoAWidget = Widget.extend({
        ...
        boundLink: function (e) {
            ...
            return self
                ...
                .then(function (result) {
                    if (
                        !Object.prototype.hasOwnProperty.call(result[1], "company_id")
                    ) {
                        self.do_warn(
                            _t("Journal items is not available for current report")
                        );
                        return;
                    }
        ...
        fold: function (e) {
            ...
            $(e.target)
                .parents("tr")
                .find("span.o_coa_foldable")
                .replaceWith(QWeb.render("unfoldable", {lineId: active_id}));
            $(e.target).parents("tr").toggleClass("o_coa_unfolded");
        },
        autounfold: function (target) {
            var self = this;
            var $CurretElement = $(target).parents("tr").find("td.treeview-td");
            ...
            this._rpc({
                ...
                kwargs: {
                    ...
                    level: parseInt(row_level, 10) + 1 || 1,
                },
            }).then(function (lines) {
