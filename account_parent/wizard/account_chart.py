...
class OpenAccountChart(models.TransientModel):
    ...
    def build_domain_context(self, wiz_id=None, account_id=None):
        ...
        if account_id:
            ...
            query = (
                'SELECT "account_move_line".id FROM ' + tables + "WHERE" + where_clause
            )
            self.env.cr.execute(SQL(query), tuple(where_params))
    ...
    def _amount_to_str(self, value, currency):
        """workaround to apply the float rounding logic of t-esc on
        data prepared server side"""
        return self.env["ir.qweb.field.monetary"].value_to_html(
            value, {"display_currency": currency}
        )
    ...
    def _float_html_formating(self, value, company):
        html_formating = True
        if (
            "output_format" in self._context.keys()
            and self._context.get("output_format") == "xls"
        ):
            html_formating = False
        return (
            html_formating and self._amount_to_str(value, company.currency_id) or value
        )
    ...
    def line_data(self, level, parent_id, wiz_id=False, account=False):
        return {
            ...
            "initial_balance": self._float_html_formating(
                account.initial_balance, account.company_id
            ),
            "ending_balance": self._float_html_formating(
    ...
    @api.model
    def _lines(self, wiz_id=None, line_id=None, level=1, obj_ids=None):
        final_vals = []
        ...
        parent_account_types = [
            {
                ...
                "id": -1001,
                "parent_id": False,
                "internal_group": ["asset", "liability", "equity"],
                "atype": False,
            },
            {
                "name": _("Profit & Loss"),
    ...
    def at_line_data(
        ...
        data.update(
            {
                "show_initial_balance": self._context.get(
                ...
                "initial_balance": self._float_html_formating(
                    total_initial_balance, company
                ),
                "ending_balance": self._float_html_formating(
                    total_ending_balance, company
                ),
                "db": total_debit,
                "cr": total_credit,
                "bal": total_balance,
    ...
    def _at_lines(self, wiz_id, line_id, level=1):
        ...
        if not line_id:
            line_id = False
        at_datas = list(
            filter(lambda x: x["parent_id"] == line_id, self.account_type_data())
        )
        ...
        if not at_datas:
            at_datas = list(
                filter(lambda x: x["id"] == line_id, self.account_type_data())
            )
            for at_data in at_datas:
                accounts = self.get_at_accounts(at_data, context)
                final_vals += self._lines(
                    wiz_id, line_id, level=level, obj_ids=accounts
                )
        return final_vals
    ...
    def get_lines(self, wiz_id=None, line_id=None, **kw):
        ...
        # html_formating = True
        # 		if 'output_format' in self._context.keys()
        #           and context.get('output_format') == 'xls':
        # 			html_formating = False
    ...
    def get_pdf(self, wiz_id):
        ...
        body = (
            ...
            ._render_template(
                ...
                values=dict(
                    ...
                    #   self.with_context(tz=self.env.user.tz), t),
                    report=self,
                    context=self,
                ),
            )
        ...
        header = self.env["ir.actions.report"]._render_template(
            "web.minimal_layout", values=dict(rcontext, subst=True, body=header)
        )
    ...
    def get_heading(self, context):
        ...
        if context.get("company_id"):
            res = (
                "Chart of Account: %s"
                % self.env["res.company"].browse(context.get("company_id")).display_name
            )
        return res
    ...
    def _get_html(self):
        ...
        result["html"] = self.env.ref("account_parent.report_coa_hierarchy")._render(
            rcontext
        )
        return result
