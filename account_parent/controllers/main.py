...
class CoAReportController(http.Controller):
    @http.route(
        "/account_parent/<string:output_format>/<string:report_name>/<int:report_id>",
        type="http",
        auth="user",
    )
...
class ExcelExportView(ExcelExport):
    ...
    def from_data(self, fields, rows):
        if len(rows) > 65535:
            raise UserError(
                _(
                    "There are too many rows (%s rows, limit: 65535) to export as "
                    "Excel 97-2003 (.xls) format. Consider splitting the export."
                )
                % len(rows)
            )
            ...
            for cell_index, cell_value in enumerate(row):
                ...
                if isinstance(cell_value, bytes) and not isinstance(
                    ...
                    try:
                        ...
                        raise UserError(
                            _(
                                "Binary fields can not be exported to Excel "
                                "unless their content is base64-encoded. "
                                "That does not seem to be the case for %s."
                            )
                            % fields[cell_index]
                        )
    ...
    @http.route(
        "/account_parent_xls/<string:output_format>"
        + "/<string:report_name>/<int:report_id>",
        type="http",
        auth="user",
    )
    def export_xls_view_parent(self, output_format, data, token, report_id=False, **kw):
        ...
        user_context = (
            request.env["account.open.chart"].browse(report_id)._build_contexts()
        )
        lines = (
            request.env["account.open.chart"]
            .with_context(print_mode=True, output_format=output_format)
        ...
        if show_initial_balance:
            ...
            row_data.append(
                ["Code", "Name", "Type", "Debit", "Credit", "Balance", "Unfoldable"]
            )
        for line in lines:
            ...
            if show_initial_balance:
                ...
                row_data.append(
                    [code, name, ac_type, debit, credit, balance, unfoldable]
                )
